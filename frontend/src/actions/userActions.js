import { USER_DETAILS_FAIL, USER_DETAILS_REQUEST, USER_DETAILS_SUCCESS,USER_DETAILS_RESET, USER_LOGIN_FAIL, USER_LOGIN_REQUEST, USER_LOGIN_SUCCESS, USER_LOGOUT, USER_REGISTER_FAIL, USER_REGISTER_REQUEST, USER_REGISTER_SUCCESS, USER_UPDATE_FAIL, USER_UPDATE_REQUEST, USER_UPDATE_SUCCESS, USER_LIST_REQUEST, USER_LIST_SUCCESS, USER_LIST_FAIL, USER_LIST_RESET, USER_DELETE_REQUEST, USER_DELETE_SUCCESS, USER_DELETE_FAIL, USER_UPDATEE_REQUEST, USER_UPDATEE_SUCCESS, USER_UPDATEE_RESET, USER_UPDATEE_FAIL } from "../constants/userConstants"
...
export const login = (email,password) =>async(dispatch)=>{
    try {
        ...
        const config = {
            headers: {  //porque el token y la info de login se estara enviando por los headers
...
export const updateUser = (user) =>async(dispatch,getState)=>{
    try {
        ...
        dispatch({ type: USER_UPDATEE_SUCCESS })

        dispatch({type: USER_DETAILS_SUCCESS, payload: data})
        dispatch({ type: USER_DETAILS_RESET })
    } catch (error) {
        dispatch({type:USER_UPDATEE_FAIL,payload: error.response && error.response.data.message ? error.response.data.message  : error.message,})
    }
}